...
def execute(argv=None, settings=None):
    ...
    settings.setdict(cmd.default_settings, priority='command')  # 把cmd.default_settings（这里为空）加入setting对象的attribute中
    cmd.settings = settings  # 把setting对象附加到命令对象上
    cmd.add_options(parser)  # 添加解析规则
    opts, args = parser.parse_args(args=argv[1:])
    _run_print_help(parser, cmd.process_options, args, opts)

    cmd.crawler_process = CrawlerProcess(settings)  # 初始化CrawlerProcess实例，把实例添加到当前命令中
