...
def _inlineCallbacks(result, g, status):
    ...
    这个函数应该是相当于yield_from的Task.step函数，用来驱动生成器。
    在下面的gotResult中会看到重新调用了_inlineCallbacks，就相当于
    yield_from中的next_future.add_done_callback(self.step)，都是
    用来驱动生成器。
    其实流程一样的，只是这里更难理解，而且addboth函数居然会调用callback，
    很费解，暂时按照yield_from理解。
    ...
    while 1:
        try:
            ...
            if isFailure:
                ...
                """
                以初始化engine那里为例来说明：
                1、在调用CrawlerRunner.crawl的时候实例化一个crawler，然后在_crawl中d = crawler.crawl(*args, **kwargs)，
                这个步骤就会使程序运行到这里来（@defer.inlineCallbacks装饰了Crawler.crawl，实际上调用crawler.crawl是
                调用的inlineCallbacks中返回的_cancellableInlineCallbacks）
                2、第一次调用的时候会激活g（这个g其实就是原生的那个crawl函数，即没有被装饰过的crawl函数，是个生成器）
                """
                result = g.send(result)
        except StopIteration as e:  # 这里不是yield from，所以每个生成器结束都会抛这个异常
            # fell off the end, or "return" statement
            status.deferred.callback(getattr(e, "value", None))  # defer结束，调用回调
            return
        ...
        if isinstance(result, Deferred):  # 如果返回对象是Defered对象（比如第一次yield self.engine.open_spider）
            ...
            result.addBoth(gotResult)  # 如果返回的是defered，添加回调。这个里面的逻辑比较重要，如果是个完成了的defered，这里面会直接执行gotResult
            if waiting[0]:  # 如果waiting[0]为True，说明这个defer还在等待(waiting)（即该defer还没有结束）
                # Haven't called back yet, set flag so that we get reinvoked
                # and return from the loop
                waiting[0] = False
...
def _cancellableInlineCallbacks(g):
    ...
    _inlineCallbacks(None, g, status)  # 第一次传入的result是None，目的就是为了激活生成器g
    return deferred
