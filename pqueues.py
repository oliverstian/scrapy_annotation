...
class ScrapyPriorityQueue(PriorityQueue):
    ...
    从PriorityQueue类的pop和push方法来看，优先级队列的意思应该是：
    有个self.queue字典即self.queues = {}，用来保存不同优先级的队列，
    优先级数字为键，该优先级对应一个内存队列类或磁盘队列类的实例，例如，
    {0: mqclass()}（mqclass为内存队列类实例）
    """
    ...
    def push(self, request, priority=0):
        if self.serialize:  # 是否序列化，序列化：把一个对象转变为一个可保存的对象（比如字典就可以保存为json格式）
