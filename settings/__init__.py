...
class BaseSettings(MutableMapping):
    ...
    def __init__(self, values=None, priority='project'):
        self.frozen = False
        self.attributes = {}  # 这个字典保存了所有的配置文件中的配置项，以配置文件的属性名为key，配置文件的属性值和优先级组成的对象为值
    ...
    def getwithbase(self, name):
        ...
        """
        compbs = BaseSettings()
        compbs.update(self[name + '_BASE'])  # name后面加个_BASE，比如DOWNLOAD_HANDLERS_BASE
    ...
    def setmodule(self, module, priority='project'):
        ...
        for key in dir(module):  # dir返回模块的所有属性
            if key.isupper():
    ...
    def update(self, values, priority='project'):
        ...
        if values is not None:
            if isinstance(values, BaseSettings):  # 类似SPIDER_MIDDLEWARES_BASE这种字典就会新建一个BaseSettings来保存
                for name, value in six.iteritems(values):  # 字典的键和值
...
class Settings(BaseSettings):
    ...
    def __init__(self, values=None, priority='project'):
        ...
        self.setmodule(default_settings, 'default')  # 把default_settins模块的所有配置项按照default的优先级保存在setting对象中
