...
class DownloaderMiddlewareManager(MiddlewareManager):
    ...
    def download(self, download_func, request, spider):
        ...
        def process_request(request):
            for method in self.methods['process_request']:  # 挨个执行下载器中间件的process_request方法
                response = yield method(request=request, spider=spider)  # 看，process_request方法就是在此处把requests和spider传入的
            ...
            defer.returnValue((yield download_func(request=request, spider=spider)))  # 如果下载器中间件没有返回值，则执行注册进来的方法，也就是Downloader的_enqueue_request
        ...
        deferred = mustbe_deferred(process_request, request)  # 这里会马上调用process_request，处理完各个中间件的process_request方法，然后将request发出下载
