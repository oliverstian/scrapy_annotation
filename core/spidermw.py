...
class SpiderMiddlewareManager(MiddlewareManager):
    ...
    def _add_middleware(self, mw):
        super(SpiderMiddlewareManager, self)._add_middleware(mw)  # 这里判断是否有open_spider，close_spider等方法
    ...
    def scrape_response(self, scrape_func, response, request, spider):
        ...
        def process_spider_input(response):  # 即引擎把response交给spider的时候，经过爬虫中间件，调用这个函数处理
            for method in self.methods['process_spider_input']:  # 执行一系列爬虫中间件的process_spider_input
                try:
            ...
            return scrape_func(response, request, spider)  # 中间件处理之后，这里注册一个事件到事件循环，等到调用爬虫的parse方法
        ...
        dfd = mustbe_deferred(process_spider_input, response)  # 马上调用process_spider_input
        dfd.addCallbacks(callback=process_spider_output, errback=process_spider_exception)  # 处理爬虫中间件的process_spider_output
        return dfd

    def process_start_requests(self, start_requests, spider):
        """
        把爬虫中间件中写了process_start_requests的添加到回调，可能就是说的requests预处理
        """
